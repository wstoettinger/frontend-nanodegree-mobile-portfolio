...
var ALL_ADJECTIVES=["dark morbid scary spooky gothic deviant creepy sadistic black dangerous dejected haunted morose tragic shattered broken sad melancholy somber dark gloomy homicidal murderous shady misty dusky ghostly shadowy demented cursed insane possessed grotesque obsessed".split(" "),"blue green purple grey scarlet NeonGreen NeonBlue NeonPink HotPink pink black red maroon silver golden yellow orange mustard plum violet cerulean brown lavender violet magenta chestnut rosy copper crimson teal indigo navy azure periwinkle brassy verdigris veridian tan raspberry beige sandy ElectricBlue white champagne coral cyan".split(" "),"whimsical silly drunken goofy funny weird strange odd playful clever boastful breakdancing hilarious conceited happy comical curious peculiar quaint quirky fancy wayward fickle yawning sleepy cockeyed dizzy dancing absurd laughing hairy smiling perplexed baffled cockamamie vulgar hoodwinked brainwashed".split(" "),
"sapphire opal silver gold platinum ruby emerald topaz diamond amethyst turquoise starlit moonlit bronze metal jade amber garnet obsidian onyx pearl copper sunlit brass brassy metallic".split(" "),"untuned loud soft shrieking melodious musical operatic symphonic dancing lyrical harmonic orchestral noisy dissonant rhythmic hissing singing crooning shouting screaming wailing crying howling yelling hollering caterwauling bawling bellowing roaring squealing beeping knocking tapping rapping humming scatting whispered whispering rasping buzzing whirring whistling whistled".split(" "),
"nuclear apocalyptic desolate atomic zombie collapsed grim fallen collapsed cannibalistic radioactive toxic poisonous venomous disastrous grimy dirty undead bloodshot rusty glowing decaying rotten deadly plagued decimated rotting putrid decayed deserted acidic".split(" "),"stupid idiotic fat ugly hideous grotesque dull dumb lazy sluggish brainless slow gullible obtuse dense dim dazed ridiculous witless daft crazy vapid inane mundane hollow vacuous boring insipid tedious monotonous weird bizarre backward moronic ignorant scatterbrained forgetful careless lethargic insolent indolent loitering gross disgusting bland horrid unseemly revolting homely deformed disfigured offensive cowardly weak villainous fearful monstrous unattractive unpleasant nasty beastly snide horrible syncophantic unhelpful bootlicking".split(" "),
...
ADJECTIVE_GROUPS="dark color whimsical shiny noise apocalyptic insulting praise scientific".split(" "),NOUN_GROUPS="animals everyday fantasy gross horror jewelry places scifi".split(" ");function randomName(){var a=Math.random(),b=ALL_ADJECTIVES[Math.floor(a*ADJECTIVE_GROUPS.length)],b=b[Math.floor(a*b.length)].capitalize(),c=ALL_NOUNS[Math.floor(a*NOUN_GROUPS.length)],a=c[Math.floor(a*c.length)].capitalize();return"The "+b+" "+a}
var makeRandomPizza=function(){for(var a=[Math.random(),Math.random(),Math.random(),Math.random()],b=Math.floor(4*a[0]),c=Math.floor(3*a[1]),d=Math.floor(2*a[2]),e=[],f=0;f<b;f++)e.push(pizzaIngredients.meats[Math.floor(a[f]*pizzaIngredients.meats.length)]);for(b=0;b<c;b++)e.push(pizzaIngredients.nonMeats[Math.floor(a[b]*pizzaIngredients.meats.length)]);for(c=0;c<d;c++)e.push(pizzaIngredients.cheeses[Math.floor(a[c]*pizzaIngredients.meats.length)]);e.push(pizzaIngredients.sauces[Math.floor(a[0]*pizzaIngredients.meats.length)]);
e.push(pizzaIngredients.crusts[Math.floor(a[1]*pizzaIngredients.meats.length)]);a="";for(d=0;d<e.length;d++)a+="<li>"+e[d]+"</li>";return a},resizePizzas=function(a){window.performance.mark("mark_start_resize");a:switch(a){case "1":document.querySelector("#pizzaSize").innerHTML="Small";break a;case "2":document.querySelector("#pizzaSize").innerHTML="Medium";break a;case "3":document.querySelector("#pizzaSize").innerHTML="Large";break a;default:console.log("bug in changeSliderLabel")}sizeStyle.innerHTML=
".mover { width: "+function(a){switch(a){case "1":return 50;case "2":return 100;case "3":return 150;default:return 100}}(a)+"px;}";window.performance.mark("mark_end_resize");window.performance.measure("measure_pizza_resize","mark_start_resize","mark_end_resize");a=window.performance.getEntriesByName("measure_pizza_resize");console.log("Time to resize pizzas: "+a[0].duration+"ms")},HTMLpizzaElement='<div is="my-pizza" id="pizza%nr%" class="pizza"><p class="name">%name%</p><ul class="ingredients">%ingreds%</ul></div>';
function generateRandomPizzas(){window.performance.mark("mark_start_generating");for(var a="",b=2;100>b;b++)a+=HTMLpizzaElement.replace("%nr%",b).replace("%name%",randomName()).replace("%ingreds%",makeRandomPizza());$("#randomPizzas").append(a);window.performance.mark("mark_end_generating");window.performance.measure("measure_pizza_generation","mark_start_generating","mark_end_generating");a=window.performance.getEntriesByName("measure_pizza_generation");console.log("Time to generate pizzas on load: "+
a[0].duration+"ms")}setTimeout(generateRandomPizzas,0);var frame=0;function logAverageFrame(a){for(var b=a.length,c=0,d=b-1;d>b-11;d--)c+=a[d].duration;console.log("Average time to generate last 10 frames: "+c/10+"ms")}var lastScrollY=0,scheduledAnimationFrame=!1;
function updatePositions(){frame++;window.performance.mark("mark_start_frame");for(var a=0;a<movers.length;a++){var b=Math.floor(50*Math.sin(lastScrollY/1250+a));movers[a].style.transform="translateX("+b+"px)"}window.performance.mark("mark_end_frame");window.performance.measure("measure_frame_duration","mark_start_frame","mark_end_frame");0===frame%10&&(a=window.performance.getEntriesByName("measure_frame_duration"),logAverageFrame(a));scheduledAnimationFrame=!1}
window.addEventListener("scroll",function(){lastScrollY=window.scrollY;scheduledAnimationFrame||(scheduledAnimationFrame=!0,requestAnimationFrame(updatePositions))});var movers=[],sizeStyle;
document.addEventListener("DOMContentLoaded",function(){sizeStyle=document.createElement("style");sizeStyle.type="text/css";document.body.appendChild(sizeStyle);for(var a=window.innerWidth/4,b=window.innerHeight/4,c=Math.floor(10*Math.random()+1),d="",e=0;4>e;e++)for(var f=0;4>f;f++)var g=100*Math.floor(Math.sin((e+f)*c)),d=d+'<img class="mover" src="images/pizza.png" srcset="images/pizza.webp" style="top:%top%; left:%left%;">'.replace("%top%",e*b+"px").replace("%left%",f*a+a/2+g+"px");$("#movingPizzas1").append(d);
