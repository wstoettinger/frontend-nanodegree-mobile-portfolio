...
a[0].duration+"ms")}setTimeout(generateRandomPizzas,0);var frame=0;function logAverageFrame(a){for(var b=a.length,c=0,d=b-1;d>b-11;d--)c+=a[d].duration;console.log("Average time to generate last 10 frames: "+c/10+"ms")}
function updatePositions(){frame++;window.performance.mark("mark_start_frame");for(var a=0;a<movers.length;a++){var b=Math.floor(50*Math.sin(document.body.scrollTop/1250+a));movers[a].style.transform="translateX("+b+"px)"}window.performance.mark("mark_end_frame");window.performance.measure("measure_frame_duration","mark_start_frame","mark_end_frame");0===frame%10&&(a=window.performance.getEntriesByName("measure_frame_duration"),logAverageFrame(a))}window.addEventListener("scroll",updatePositions);
var movers=[],sizeStyle;
