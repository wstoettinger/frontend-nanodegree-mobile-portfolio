...
var HTMLpizzaElement = '<div is="my-pizza" id="pizza%nr%" class="pizza"><p class="name">%name%</p><ul class="ingredients" /></div>';
...
function generateRandomPizzas() {
  ...
  for (var i = 2; i < 100; i++) {
    var HTML = HTMLpizzaElement.replace("%nr%", i).replace("%name%", randomName());
    $("#randomPizzas").append(HTML).children().last().children().filter(".ingredients").append(makeRandomPizza());
  }
...
// window.addEventListener('scroll', updatePositions); // moving the pizzas while scrolling costs time and is annoying!
